...
def train_ec(model, train_loader, optimizer, criterion, num_epochs, depth_areas, device,
    ...
    for epoch in tqdm(range(num_epochs), desc='# epochs'):
        ...
        for batch_num, batch in enumerate(tqdm(train_loader, desc='# batches', leave=False)):
            ...
            # [~torch.isnan(loss_Y)] for not including nan-values
            # in labels used for calculating MSE-loss
            ...
            d_loss = criterion(loss_preds[~torch.isnan(
                loss_Y)], loss_Y[~torch.isnan(loss_Y)])
