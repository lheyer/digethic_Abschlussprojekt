...
def calculate_heat_flux_sensible(surf_temp, air_temp, rel_hum, wind_speed):
    ...
    # GLM code ->  Q_sensibleheat = -CH * (rho_air * 1005.) * WindSp * (Lake[surfLayer].Temp
    # - MetData.AirTemp);
...
def calculate_heat_flux_latent(surf_temp, air_temp, rel_hum, wind_speed):
    ...
    # GLM code-> Q_latentheat = -CE * rho_air * Latent_Heat_Evap * (0.622/p_atm) * WindSp
...
def calculate_vapour_pressure_saturated(temp):
    ...
    # Converted pow function to exp function workaround pytorch
    # not having autograd implemented for pow
    exponent = torch.tensor(
        (9.28603523 - (2332.37885 / (temp + 273.15))) * math.log(10))
...
def transformTempToDensity(temp, use_gpu):
    ...
    # print(temp.size())
    densities[:] = 1000 * (
        1 - ((temp[:] + 288.9414) * torch.pow(temp[:] - 3.9863, 2))
        / (508929.2 * (temp[:] + 68.12963)))
    # densities[:] = 1000*(1-((temp[:]+288.9414)*torch.pow(temp[:]
    # - 3.9863))/(508929.2*(temp[:]+68.12963)))
    # print("DENSITIES")
    # for i in range(10):
    #     print(densities[i,i])
...
# def calculate_ec_loss(inputs, outputs, phys, labels, dates, depth_areas, n_depths, ec_threshold,
# use_gpu,
#                       combine_days=1):
def calculate_ec_loss(inputs, outputs, phys, labels, dates, depth_areas, n_depths, ec_threshold,
                      use_gpu, combine_days=1):
    """
    ...
    for i in range(n_sets):
        ...
        lake_energies = calculate_lake_energy(outputs[start_index:end_index, :],
                                              densities[start_index:end_index, :],
                                              depth_areas)
        # print('lake_energies: ',lake_energies.size())
        # calculate energy change in each timestep
        lake_energy_deltas = calculate_lake_energy_deltas(
            lake_energies, combine_days, depth_areas[0])
        ...
        lake_energy_fluxes = calculate_energy_fluxes(
            phys[start_index, :, :], outputs[start_index, :], combine_days)
...
def calculate_lake_energy(temps, densities, depth_areas):
    ...
    depth_areas = depth_areas.view(
        n_depths, 1).expand(n_depths, temps.size()[1])
...
def calculate_lake_energy_deltas(energies, combine_days, surface_area):
    ...
    #     energy_deltas[t-1] = (energies[t+combine_days] - energies[t])/(time*surface_area)
    # #energy difference converted to W/m^2
...
def calculate_energy_fluxes(phys, surf_temps, combine_days):
    ...
    R_lw_out_arr = R_lw_out_arr[:-1] + \
        (R_lw_out_arr[1:] - R_lw_out_arr[:-1]) / 2
    ...
    # test
    fluxes = (R_sw_arr[:-1] * (1 - alpha_sw) + R_lw_arr[:-1]
              * (1 - alpha_lw) - R_lw_out_arr[:-1] + E[:-1] + H[:-1])
